...
//---------------------------------------
#if defined(USE_UNITY_STANDARD_INPUT_DEFINES)
    half4       _Color;
...
// Some settings that can't be removed yet.
// UVSec can be removed once a more rebost way of passing the extra UVs is done.
half        _UVSec;
half        _Cutoff;
...
struct VertexInput
{
    ...
    #endif
#else // USE_UNITY_STANDARD_INPUT_DEFINES
    float4 TexCoords(VertexInput v)
    {
        float4 texcoord;
        texcoord.xy = (v.uv0); // Always source from uv0
        texcoord.zw = (((_UVSec == 0) ? v.uv0 : v.uv1));
    #if defined(HAS_ATTRIBUTE_UV2)
        texcoord.zw = (((_UVSec == 2) ? v.uv2 : texcoord.zw));
    #endif
    #if defined(HAS_ATTRIBUTE_UV3)
        texcoord.zw = (((_UVSec == 3) ? v.uv3 : texcoord.zw));
    #endif
        return texcoord;
    }
